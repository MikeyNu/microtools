...
const toolCategories = [
  ...
  {
    title: "Network Tools",
    description: "IP lookup, DNS lookup, port scanner",
    icon: Network,
    count: 3,
    href: "/network-tools",
  },
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="border-b border-border py-14 sm:py-20">
        <div className="container mx-auto px-4 sm:px-6">
          <h1 className="font-mono font-bold text-5xl sm:text-6xl text-foreground tracking-tight leading-none mb-5 max-w-2xl">
      ...
      {/* ── Ad slot ──────────────────────────────────────────────── */}
      {shouldDisplayAds() && (
        <div className="border-b border-border py-6 bg-muted/30">
              ...
              <nav className="space-y-0.5 max-h-[calc(100vh-10rem)] overflow-y-auto">
                {toolCategories.map((cat) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className="h-3.5 w-3.5 flex-shrink-0 group-hover:text-accent transition-colors" />
                ...
                {toolCategories.map((cat) => {
                  ...
                  return (
                    <Link
                      key={cat.href}
                      href={cat.href}
                      className="flex-shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border border-border bg-card text-xs font-mono text-muted-foreground hover:text-foreground hover:border-accent/50 transition-colors"
                    >
                ...
                {toolCategories.map((category) => {
                  ...
                  return (
                    <Link key={category.href} href={category.href} className="group">
                      <div className="h-full rounded-lg border border-border bg-card p-5 transition-all duration-200 hover:border-accent/50 hover:shadow-md">
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-9 h-9 rounded-md bg-accent flex items-center justify-center flex-shrink-0">
                            <Icon className="h-4.5 w-4.5 text-accent-foreground" style={{ width: '1.125rem', height: '1.125rem' }} />
                          </div>
                          <span className="text-xs font-mono text-muted-foreground tabular-nums">
                            {category.count}
                          </span>
                        </div>
                        <h2 className="font-mono font-semibold text-sm text-foreground mb-1.5 tracking-tight group-hover:text-accent transition-colors duration-200">
                          {category.title}
                        </h2>
            ...
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-2 mb-2">
                <svg
                  ...
                  aria-hidden="true"
                >
                  <polygon fill="#C13209" points="0 114.848 159.404 203.960 316.653 116.662 316.653 265.181 185.253 340.008 185.253 299.194 282.301 244.321 282.301 175.843 159.404 246.135 36.280 176.070 36.280 244.094 132.874 298.740 132.874 340.008 0 264.274 0 114.848" />
                  <path fill="#C13209" d="M159.290,0 L2.154,90.699 l157.249,87.525 157.250,-87.525 L159.290,0 Z M188.541,105.721 v-26.303 h-58.954 v26.303 h-19.727 v-30.157 c0-9.330,7.563-16.893,16.893-16.893 h65.757 c9.330,0,16.893,7.563,16.893,16.893 v30.157 h-20.861 Z" />
                </svg>
                <span className="font-mono font-bold text-foreground text-base leading-none">
