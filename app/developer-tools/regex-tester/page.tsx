...
Password: MySecure123!`
  ...
  const highlightMatches = (text: string, matches: Match[]) => {
    ...
    matches.forEach((match, i) => {
      ...
      result += `<mark class="bg-warning/20 px-1 rounded">${match.match}</mark>`
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {commonPatterns.map((patternData, index) => (
              <div key={index} className="p-3 border rounded-lg hover:bg-muted/50 cursor-pointer"
                ...
                <h4 className="font-medium text-foreground mb-1">{patternData.name}</h4>
                <p className="text-xs text-muted-foreground mb-2">{patternData.description}</p>
